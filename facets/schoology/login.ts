// Disabled: Schoology's three-legged OAuth is broken for new tokens (the authorize page
// won't hand them out), so the tracker is two-legged for now: SCHOOLOGY_CONSUMER_KEY /
// SCHOOLOGY_CONSUMER_SECRET alone, acting as the key's owner. Kept for when it works again;
// it was run as `pnpm schoology:login` ("schoology:login": "node facets/schoology/login.ts").
